...
internal class QARequestServiceTest {
    ...
    @Test
    fun `Get qa requests, should return pagination list`() {
    ...
    @Test
    fun `Get my qa requests, should return pagination list`() {
        val mockRequestPage = PageImpl<QARequest>(listOf(mockQARequest))
        `when`(qaRequestRepository.findAllByEmail(anyString(), any(Pageable::class.java))).thenReturn(mockRequestPage)

        val qaRequestTO = mockQARequestTO
        qaRequestTO.id = mockQARequest.id.toString()
        val mockRequestTOPage = PageImpl<QARequestTO>(listOf(qaRequestTO))
        val resultPage = qaRequestService.getMyRequests(PageRequest.of(1, 1), testEmail)
    ...
    @Test
    fun `Create qa request, should return correct qaRequestTO`() {
    ...
    @Test
    fun `Create qa request, should throw error when qaRequestTo is null`() {
    ...
    @Test
    fun `Edit qa request, should return updated qaRequestTo`() {
    ...
    @Test
    fun `Edit qa request, should throw error when failed to find entity`() {
    ...
    @Test
    fun `Edit qa request, should throw error when failed to save entity`() {
        `when`(qaRequestRepository.findById(any(UUID::class.java))).thenReturn(mockOptionalQARequest)
        `when`(qaRequestRepository.save(any(QARequest::class.java))).thenThrow(IllegalArgumentException())
    ...
    @Test
    fun `Delete qa request, should return true`() {
        `when`(qaRequestRepository.findById(any(UUID::class.java))).thenReturn(mockOptionalQARequest)
    ...
    @Test
    fun `Delete qa request, should return false when cannot delete`() {
        `when`(qaRequestRepository.findById(any(UUID::class.java))).thenReturn(mockOptionalQARequest)
    ...
    @Test
    fun `Archive qa request, should work without any exception`() {
        `when`(qaRequestRepository.findById(any(UUID::class.java))).thenReturn(mockOptionalQARequest)
        `when`(qaRequestRepository.save(any(QARequest::class.java))).thenReturn(mockQARequest)
    ...
    @Test
    fun `Archive qa request, should throw not found exception`() {
        `when`(qaRequestRepository.findById(any(UUID::class.java))).thenThrow(QARequestNotFoundException())
    ...
    @Test
    fun `Archive qa request, should throw exception`() {
        `when`(qaRequestRepository.findById(any(UUID::class.java))).thenReturn(mockOptionalQARequest)
        `when`(qaRequestRepository.save(any(QARequest::class.java))).thenThrow(IllegalArgumentException())
